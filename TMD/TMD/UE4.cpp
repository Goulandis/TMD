...
void UE4::Cook(vector<string>::iterator it, string& cmdexe, string cmd)
{
	...
	if (*it == Andriod_ASTC)
	{
		cmdexe = UE4Editor + " " + UProject + " -run=Cook -TargetPlatform=Android_ASTC -fileopenlog -unversioned -abslog=" + CookLog + " -stdout -CrashForUAT -unattended -NoLogTimes -UTF8Output";
	}
	else if (*it == WindowsNoEditor)
	{
		cmdexe = UE4Editor + " " + UProject + " -run=Cook -TargetPlatform=WindowsNoEditor -fileopenlog -unversioned -abslog=" + CookLog + " -stdout -CrashForUAT -unattended -NoLogTimes -UTF8Output";
	}
	else if (*it == Andriod_ETC1)
	{
		cmdexe = UE4Editor + " " + UProject + " -run=Cook -TargetPlatform=Android_ETC1 -fileopenlog -unversioned -abslog=" + CookLog + " -stdout -CrashForUAT -unattended -NoLogTimes -UTF8Output";
	}
	else
	{
		Output::OutSyntaxError(cmd);
